...
def send_to_backend(image, mode, name, password):
    with st.spinner(f"Memproses {mode}..."):
        try:
            ...
            # Send request
            if mode == "Register":
                response = requests.post(f"{BACKEND_URL}/api/face/register/", files=files, data=data)
            else:
                response = requests.post(f"{BACKEND_URL}/api/face/login/", files=files, data=data)
...
def delete_user(user_id):
    try:
        response = requests.delete(f"{BACKEND_URL}/api/face/{user_id}")
        if response.status_code == 200:
        ...
        if not name or not password:
            ...
            if uploaded_file is not None:
                ...
                if len(faces) == 0:
                    ...
                    # Pastikan menggunakan gambar asli, bukan yang sudah dianotasi
                    if send_to_backend(uploaded_image, mode, name, password):
                        # Jika berhasil, refresh halaman
                        st.experimental_rerun()
            elif st.session_state["captured_image"] is not None:
                # Gunakan gambar hasil capture
                captured_img = st.session_state["captured_image"]
                # Pastikan gambar dalam format BGR (OpenCV default)
